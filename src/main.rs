...
fn print_usage() {
    ...
    eprintln!(
        "Pass --commit-checkpoints to emit the checkpoint trace file at the end of the run. Pass --terminal-checkpoint to stop after a named checkpoint such as prefill.finalize. Pass --raster to use raster-authored tiles where implemented."
    );
}
