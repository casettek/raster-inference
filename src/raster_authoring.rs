...
pub mod prelude {
    pub use crate::{
        call, call_seq, external, raster_authoring::sequence, raster_authoring::tile,
...
mod tests {
    ...
    #[test]
    fn call_invokes_tile_function() {
        assert_eq!(call!(add_one, 41), 42);
    }
    ...
    fn recursive_tile_macro_runs_until_done_for_single_state() {
        assert_eq!(double_until_at_least_ten!(2), 16);
    }

    #[test]
    fn recursive_tile_macro_runs_until_done_for_tuple_state() {
        assert_eq!(count_to!(0, 3), (3, 3));
    }
