...
pub fn run(
    ...
    if native_tiles::check_stop_condition(decode_state.generated_token_ids.len(), max_new_tokens)
        .is_some()
    {
        return Ok(None);
    }
    ...
    let next_token = native_tiles::select_next_token_internal(&logits, execution_mode)?;
    decode_state.full_token_ids =
        native_tiles::append_token(&decode_state.full_token_ids, next_token);
    decode_state.generated_token_ids =
        native_tiles::append_token(&decode_state.generated_token_ids, next_token);
...
fn decode_select_checkpoint_state(
    ...
    Ok(json!({
        ...
        "generated_token_ids_sha256": crate::output_finalize::native_tiles::build_output_decode_commitment(&decode_state.generated_token_ids)?,
