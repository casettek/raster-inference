...
pub fn run(decode_state: DecodeState, tokenizer: &Tokenizer) -> Result<OutputDecodeState> {
    ...
    let generated_text =
        native_tiles::detokenize_output_tokens(tokenizer, &decode_state.generated_token_ids)?;
    let generated_token_ids_sha256 =
        native_tiles::build_output_decode_commitment(&decode_state.generated_token_ids)?;
...
pub fn run_raster_with_roots(
    ...
    let generated_text = raster_auth_source::materialize_text_from_roots(
        &refs.artifact_store_roots,
