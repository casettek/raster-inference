...
//!
//! Data placement (trace-slimming refactor, D13): the model-scoped merge
//! table stays an `AuthRef` all the way into the scan recur *sequence*,
//! whose per-chunk tile receives each chunk as an external-selection
//! binding — the chunk bytes never ride the trace inline. Only the
//! prompt-scoped round/iteration contexts ride `args = (…)`.
...
/// One BPE merge round: priority-order merge scan → merge decision →
/// chunked pieces rebuild → next loop state.
#[sequence(kind = recur)]
